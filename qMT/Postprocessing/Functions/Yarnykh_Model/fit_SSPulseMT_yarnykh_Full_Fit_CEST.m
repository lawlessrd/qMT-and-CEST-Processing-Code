function [x,chi2,chi2p,res,resn,out,exit] = fit_SSPulseMT_yarnykh_Full_Fit_CEST(x0,Mdat,pwMT,ts,TR,R1obs,theta,w1e,delta,lineshape)
...
[x,resn,res,exit,out,lam,J] = ...
    lsqnonlin(@fitModel,x0,[1e-3 1e-3 1e-4 1e-9],[1 100 1 1]...
...
[Mact,Mnorm] = yarnykh_pulseMT_CEST(M0,R1,T2,TR,kba,pwMT,ts,thetaEX,delta,w1e,lineshape);
Mact = double(Mact);
Mnorm = double(Mnorm);
